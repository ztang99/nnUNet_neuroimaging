...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, modal:list, patch_size, final_patch_size, batch_size, has_prev_stage=False,
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # selected_idx = self.modal + [-1]
            selected_idx = np.random.choice(modality, 1, True, None).tolist()
            selected_idx += [-1]
            ...
            if isfile(self._data[i]['data_file'][:-4] + ".npy"):
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode)[selected_idx, ...]
            else:
                case_all_data = np.load(self._data[i]['data_file'])['data'][selected_idx, ...]
