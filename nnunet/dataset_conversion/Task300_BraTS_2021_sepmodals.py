...
if __name__ == "__main__":
    ...
    task_name = "Task400_BraTS2021"
    downloaded_data_dir = "/home/aris_data/PublicBrainTumorData/BRATS2021_glioma_seg"
    # downloaded_data_dir_val = "/home/fabian/Downloads/MICCAI_BraTS2020_ValidationData"

    subjects = subdirs(downloaded_data_dir)
    from sklearn.model_selection import train_test_split

    downloaded_data_dir, downloaded_data_dir_val = train_test_split(subjects, test_size=0.25, random_state=42)
    ...
    patient_names = []
    for p in downloaded_data_dir:
        patdir = p
        patient_name = p.split("/")[-1]
       
        t1 = join(patdir, "T1_stripped.nii.gz")
        t1c = join(patdir, "T1c_stripped.nii.gz")
        t2 = join(patdir, "T2_stripped.nii.gz")
        flair = join(patdir, "Flair_stripped.nii.gz")
        seg = join(patdir, "OTMultiClass.nii.gz")
        ...
        patient_names.append(patient_name + "T1" )
        patient_names.append(patient_name + "T1c" )
        patient_names.append(patient_name + "T2" )
        patient_names.append(patient_name + "Flair" )

        """
        Also some of my thoughts: I do think we need to change the model as well? 
        Oh wait... maybe just self.modal = 0 all the time 
        Can I just use nnUNetTrainer_T1.py? and in train_nnunet_T1.sh just change the Task name I think should work
        """
        ...
        shutil.copy(t1, join(target_imagesTr, patient_name  + "T1" + "_0000.nii.gz"))
        shutil.copy(t1c, join(target_imagesTr, patient_name + "T1c" + "_0000.nii.gz"))
        shutil.copy(t2, join(target_imagesTr, patient_name + "T2" + "_0000.nii.gz"))
        shutil.copy(flair, join(target_imagesTr, patient_name + "Flair" + "_0000.nii.gz"))
        
        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, patient_name + "T1" + ".nii.gz"))
        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, patient_name + "T1c" + ".nii.gz"))
        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, patient_name + "T2" + ".nii.gz"))
        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, patient_name + "Flair" + ".nii.gz"))
    ...
    if downloaded_data_dir_val is not None:
        for p in downloaded_data_dir_val:
            patdir = p
            patient_name = p.split("/")[-1]
            t1 = join(patdir, "T1_stripped.nii.gz")
            t1c = join(patdir, "T1c_stripped.nii.gz")
            ...
            shutil.copy(t1, join(target_imagesTs, patient_name  + "T1" + "_0000.nii.gz"))
            shutil.copy(t1c, join(target_imagesTs, patient_name + "T1c" + "_0000.nii.gz"))
            shutil.copy(t2, join(target_imagesTs, patient_name + "T2" + "_0000.nii.gz"))
            shutil.copy(flair, join(target_imagesTs, patient_name + "Flair" + "_0000.nii.gz"))
            
            copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTs, patient_name + "T1" + ".nii.gz"))
            copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTs, patient_name + "T1c" + ".nii.gz"))
            copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTs, patient_name + "T2" + ".nii.gz"))
            copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTs, patient_name + "Flair" + ".nii.gz"))